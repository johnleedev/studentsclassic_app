...
function LessonPost(props: any) {

  const [title, setTitle] = useState('');
  const [content, setContent] = useState('');
  const [currentTab, setCurrentTab] = useState(1);
  const [sort, setSort] = useState('나도한마디');
  ...
  const createPost = async () => {
    ...
    if (post !== null && editMode !== null) {
      // 기존 글 수정 처리
      axios
        .post(`${MainURL}/lessons/posts/${post.id}/edit`, {
          title: title, content: content
        })
        .then((res) => {
          if (res.data === true) {
            Alert.alert('수정되었습니다.');
            props.navigation.replace('LessonMain');
          } else {
      ...
      <ScrollView style={{flex:1}}>
        ...
        <View style={styles.section}>
          ...
          <View style={{flexDirection:'row', marginBottom:10}}>
            <TouchableOpacity
              style={{flexDirection:'row', alignItems:'center', borderWidth:1, borderRadius:5, padding:10, marginRight:10,
                      borderColor: currentTab === 1 ? '#333' :'#F5F4F3' }}
              onPress={()=>{setCurrentTab(1); setSort('나도한마디')}}
            >
              <Entypo name='megaphone' color='#333' size={15} style={{marginRight:5}}/>
              <Typography color= {currentTab === 1 ? '#333' : '#6F6F6F'} fontSize={12} fontWeightIdx={2}>나도한마디</Typography>
            </TouchableOpacity>
            <TouchableOpacity 
              style={{flexDirection:'row', alignItems:'center', borderWidth:1, borderRadius:5, padding:10, marginRight:10,
                      borderColor: currentTab === 2 ? '#333' :'#F5F4F3' }}
              onPress={()=>{setCurrentTab(2); setSort('질문있어요')}}
            >
              <Ionicons name='hand-right-outline' color='#333' size={15} style={{marginRight:5}}/>
              <Typography color= {currentTab === 2 ? '#333' : '#6F6F6F'} fontSize={12} fontWeightIdx={2}>질문있어요</Typography>
            </TouchableOpacity>
          </View>
