...
export default function WordSearch (props : any) {
  ...
  const fetchPosts = () => {
    axios.get(`${MainURL}/study/getmusicword/${page}`)
    .then((res) => {
      if(res.data) {
        setIsResdataFalse(false);
        let data: any = [...res.data.items];
        let wordsCopy = [...words, ...data] 
        wordsCopy.sort((a: any, b: any) => a.word > b.word ? 1 : -1);
        setWords(wordsCopy);
  ...
  const fetchPostsSearch = (searchAlphabet:string) => {
    setWordsViewList([]);
    axios.get(`${MainURL}/study/getmusicwordsearch/${searchAlphabet}`)
    .then((res) => {
      if(res.data) {
    ...
    <View style={styles.container}>
      ...
      <View style={{paddingHorizontal:20, paddingBottom:100}}>
        ...
        <View style={{width:'100%', alignItems:'flex-end', paddingRight:10}}>
          <Typography fontSize={12} color='#8C8C8C'>총 407개의 단어가 있습니다.</Typography>
        </View>
              ...
              wordsViewList.map((item:any, index:any)=>{
                ...
                return ( 
                  <View key={index}
                    ...
                    <View style={{width:'45%', justifyContent:'flex-start', flexDirection:'row', alignItems:'center'}}>
                      <Image
